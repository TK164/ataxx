...
class BoardWidget extends Pad  {
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (int x = 0; x <= _dim; x += SQDIM) {
            g.drawLine(x, 0, x, _dim);
        }
        for (int y = 0; y <= _dim; y += SQDIM) {
            g.drawLine(0, y, _dim, y);
        }
        if (_selectedCol != 0 && _selectedRow != 0) {
            g.setColor(SELECTED_COLOR);
            int x = (_selectedCol - 'a') * SQDIM;
            int y = ('7' - _selectedRow) * SQDIM;
            g.fillRect(x, y, SQDIM, SQDIM);
        }

        for (char col = 'a'; col < 'a' + SIDE; col++) {
            for (char row = '1'; row < '1' + SIDE; row++) {
                int x = (col - 'a') * SQDIM + SQDIM / 8;
                int y = ('7' - row) * SQDIM + SQDIM / 8;
                if (_model.get(col, row) == RED) {
                    g.setColor(RED_COLOR);
                    g.fillOval(x, y, 3 * SQDIM / 4, 3 * SQDIM / 4);
    ...
    void drawBlock(Graphics2D g, int cx, int cy) {
        ...
        g.drawLine(cx, cy, cx + 3 * SQDIM / 4, cy + 3 * SQDIM / 4);
        g.drawLine(cx, cy + 3 * SQDIM / 4, cx + 3 * SQDIM / 4, cy);
    }
