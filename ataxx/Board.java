...
class Board {
    ...
    void clear() {
        ...
        unrecordedSet(EXTENDED_SIDE * 3 - 3, RED);
        unrecordedSet(EXTENDED_SIDE * (EXTENDED_SIDE - 3) + 2, RED);
        unrecordedSet(EXTENDED_SIDE * (EXTENDED_SIDE - 2)  - 3, BLUE);
        announce();
    }
     ...
     *  or if neither player can mo
     *  ve and both have the same number of pieces.*/
    ...
    boolean legalMove(Move move) {
        if (move == null) {
            ...
            return true;
        } else if (_board[index(move.col0(), move.row0())] != _whoseMove) {
            return false;
        } else if (_board[index(move.col1(), move.row1())] == BLOCKED) {
            return false;
    ...
    boolean canMove(PieceColor who) {
        if (who == RED && redPieces() == 0) {
            return false;
        } else if (who == BLUE && bluePieces() == 0) {
            return false;
        }
        for (char row = '1'; row < '1' + SIDE; row++) {
    ...
    void makeMove(Move move) {
        ...
        int linIndex = index(move.col1(), move.row1());
        if (move.isExtend()) {
            set(linIndex, _whoseMove);
            _totalOpen--;
            _numJumps = 0;
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                if (get(neighbor(linIndex, i, j)) == opponent) {
                    set(neighbor(linIndex, i, j), _whoseMove);
                    _numPieces[_whoseMove.ordinal()]++;
    ...
    boolean legalBlock(char c, char r) {
        int ind = index(c, r);
        if (ind >= _board.length || ind < 0) {
            throw error("Position does not exist on board.");
        }
    ...
    void setBlock(char c, char r) {
        ...
        int[] linIndex = new int[4];
        linIndex[0] = index(c, r);
        if (linIndex[0] % EXTENDED_SIDE <= EXTENDED_SIDE / 2) {
            linIndex[1] = linIndex[0]
                    + EXTENDED_SIDE - 2 * (linIndex[0] % EXTENDED_SIDE) - 1;
        } else {
            linIndex[1] = linIndex[0] - 2
                    * (linIndex[0] % EXTENDED_SIDE - EXTENDED_SIDE / 2);
        }
        linIndex[2] = EXTENDED_SIDE
                * (EXTENDED_SIDE - 1 - linIndex[0] / EXTENDED_SIDE)
                + linIndex[0] % EXTENDED_SIDE;
        linIndex[3] = EXTENDED_SIDE
                * (EXTENDED_SIDE - 1 - linIndex[1] / EXTENDED_SIDE)
                + linIndex[1] % EXTENDED_SIDE;

        for (int i = 0; i < linIndex.length; i++) {
            if (_board[linIndex[i]] != BLOCKED) {
                _totalOpen--;
            }
            _board[linIndex[i]] = BLOCKED;
        }
